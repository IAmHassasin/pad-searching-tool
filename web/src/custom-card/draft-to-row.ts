...
import { formatAwakeningList, type CustomCardDraft } from "./types";

export function draftToPreviewRow(draft: CustomCardDraft): MonsterRecord {
  const id = draft.number != null && Number.isFinite(draft.number) ? draft.number : 0;
  const superIds =
  ...
  return {
    ...
    active_skill_desc_en: draft.activeSkillDesc.trim() || null,
